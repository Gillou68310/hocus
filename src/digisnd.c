...
// Private globals
//
char *SoundBlasterError;
bool sndInitialized;

SoundFinishedCallback sbSoundFinishedCallback;
NewVocSectionCallback sbNewVocSectionCallback;
...
InterruptHandler sbSavedIntHandler;

int sbCodecType;
byte sbTimeValue;
volatile bool sbSamplePlaying;
volatile byte huge *sbNextChunkPtr;
volatile dword sbNextChunkLen;

int sbVocRepeatIndex;
byte huge *sbVocToRepeat[MAX_NESTED_VOC_REPEATS];
word sbVocRepeatCounts[MAX_NESTED_VOC_REPEATS];
bool sbVocPlaying;
byte huge *sbVocData;
...
static void SBL_EnableDMAInt(void)
{
    ...
    // it's unlikely to occur in practice due to the way SBL_RestoreDMAInt() is
...
static void SBL_RestoreDMAInt(void)
{
    byte interruptMask;

    // This works similary to SBL_EnableDMAInt, but here we want to set the
    // bit, not unset it.
    interruptMask = inportb(0x21);

    // This condition will be true if either SBL_EnableDMAInt() has never been
    // called so far (so sbOldIntMask is still at its initial value of all 1s),
    // or if the desired interrupt was disabled at the time SBL_EnableDMAInt()
    // was last called.
    ...
    else
    {
        ...
        // SBL_EnableDMAInt() was called. In that case, we unset the bit.  This
        // effectively keeps the interrupt enabled, or even enables it in case
        // SBL_RestoreDMAInt() is called without calling SBL_EnableDMAInt()
        // first.
        //
        ...
        // was already enabled before SBL_EnableDMAInt() was called, but I'm not
        ...
        // interrupt handler. But that variable is set in SBL_EnableDMAInt every
        ...
        // with SBL_EnableDMAInt(), not in InitSoundBlaster()).
        ...
        // 2 enabled if it was already enabled by the time SBL_EnableDMAInt()
    ...
    // Same thing here as in SBL_EnableDMAInt - use the secondary PIC if the
    // interrupt number is higher than 7. The logic here is otherwise identical
    // to the code above. Also see the [BUG] note in SBL_EnableDMAInt() on this.
    if (sbInterrupt >= 8)
    {
...
static void SBL_DoDSPCommand(byte command, word value)
{
    // Write command ID
    sbAwaitReady();
 ...
 */
static void SB_StopSample(void)
{
    int i;
 ...
 * and it doesn't use a function like SBL_DoDSPCommand but inlines the needed
 * code.
 */
    ...
    volatile byte huge *data, dword length, bool hasRefByte)
{
    ...
    SBL_DoDSPCommand(
        hasRefByte ? PLAY_CMDS_WITH_REF[sbCodecType] : PLAY_CMDS_NO_REF[sbCodecType],
        lengthToPlay);
...
static void SBL_SetFreq(byte timeValue)
{
    DISABLE_INTERRUPTS();
...
static byte SBL_Freq2TimeValue(long sampleRate)
{
...
{
    SB_PlaySample2(
        data, SBL_Freq2TimeValue(sampleRate), CODEC_8BIT_PCM, true, length);
}

/** "Play" silence - pause playback for specified duration */
static void SB_PlaySilence2(byte timeValue, dword length)
{
    DISABLE_INTERRUPTS();
    ...
    // sbNextChunkPtr is NULL, and thus call SB_StopSample.
    SBL_DoDSPCommand(CMD_PAUSE_DAC, length);
...
{
    SB_PlaySilence2(SBL_Freq2TimeValue(sampleRate), length);
}
...
static void PlayNextVocSection(void)
{
    ...
    do
    {
        ...
        case VOC_SECTION_SILENCE:
            ...
            SB_PlaySilence2(*(sbVocData + 2), *((word far *)sbVocData));
            break;
...
void SB_PlayVOC(byte huge *data, bool includesHeader)
{
    ...
    // the file.
    SB_SetSampleHook(PlayNextVocSection);
 ...
 *
 * Public function. Not used by the game, it only uses SB_SamplePlaying.
 */
...
{
    int i;
    int originalAddress = alBase;

    sbLocation = portIndex << 4;
    alBase = sbLocation + sbFMAddress;
    ...
    // similarly to what we do in SB_ALOut().
    asm mov dx, [alBase];
    asm in al, dx;
    asm in al, dx;
    asm in al, dx;
...
waitLoop:
    ...
    // Now attempt to read the data byte, and retry this 200 times in case the
...
static void InitSoundBlaster(void)
{
    ...
    // SBL_EnableDMAInt()/SBL_RestoreDMAInt().
    //
    ...
    // SBL_EnableDMAInt() and SBL_RestoreDMAInt(). That would be ok if
    ...
    // means that SBL_RestoreDMAInt() could potentially disable the entire
    // secondary PIC, i.e. IRQs 8 through 15. The only reason this doesn't happen
    // is because of the slightly odd way SBL_RestoreDMAInt() is implemented.
    // See that function for more details.
    //
    // One could also argue that this function here is correct, and that
    // SBL_EnableDMAInt/SBL_RestoreDMAInt are wrong to make use of sbIntMask
...
/** Send a command to the AdLib hardware, using alBase
 *
 ...
 */
static void SB_ALOut(byte reg, byte val)
{
    DISABLE_INTERRUPTS();
...
/** Detect an OPL2 (or compatible) device at I/O port alBase
 *
 ...
 */
static int SBL_CheckEnv(char *env)
{
    long temp;
    char c;
    ...
    while (*env)
    {
        ...
        case 'A':
            ...
            {
                SoundBlasterError = "Unsupported address value";
                return -1;
            }
            break;
        ...
        case 'I':
            ...
            {
                SoundBlasterError = "Unsupported interrupt value";
                return -1;
            }
            break;
        ...
        case 'D':
            ...
            {
                SoundBlasterError = "Unsupported DMA channel";
                return -1;
            }
            break;
...
char *SB_Startup(char *blasterEnvVar)
{
    ...
    if (blasterEnvVar)
    {
        portIndex = SBL_CheckEnv(blasterEnvVar);

        if (SoundBlasterError != NULL)
        {
            return SoundBlasterError;
        }
    }
