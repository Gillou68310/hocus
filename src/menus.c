...
void do_line_menu(unsigned char *msg, int fn)
{
    ...
    y = 107;
    pstr(x, y, 5, msg);
    restore_graphics_fragment(DB_BOTTOM, 0, 0xb8);
    pstrsh((SCREEN_WIDTH - pstrlen(footers[fn])) / 2, 0xbc, 4, footers[fn]);
    restore_graphics_fragment(DB_HOCUS, 0, 0);
    fade_in(0x14);
}
...
void do_ordering_info(void)
{
    ...
    do
    {
        load_file_to_byte_pointer(page + DB_ORDER, &inf);
...
{
    load_file_to_byte_pointer(DB_REGISTER, &inf);
    do_info_screen(0, 1);
    get_any_key();
}
